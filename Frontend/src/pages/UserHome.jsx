...
// ============================================================
// SIDEBAR — persistent left navigation. Now also carries what the
// old fixed Header used to show: the brand/title block, a live
// date/time readout, and the user account panel with sign-out —
// so the top header bar is gone and this is the app's one anchor.
// Collapsible to a 56px icon rail.
// ============================================================
function Sidebar({ collapsed, onToggleCollapse, activePath }) {
  ...
  return (
    <aside
      ...
      style={{ width }}
    >
      {/* accent strip — the header's gradient line, now anchored to the sidebar */}
      <div
        className="h-[2px] w-full flex-shrink-0"
        style={{ background: `linear-gradient(90deg, ${NAVY} 0%, ${BORDER} 50%, ${GOLD} 100%)` }}
      />

      {/* brand block — replaces the old header's left title block */}
      ...
      {/* live date / time — replaces the old header's readout panel */}
      {!collapsed ? (
      ...
      {/* user account panel — replaces the old header's account chip + logout */}
