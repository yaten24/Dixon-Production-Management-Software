...
// plus Recent / Heatmap full-page modals (same pattern as
// MouldChangeDashboard's RecentMouldChangesModal / MouldChangeHeatmap),
// restyled to the navy/gold flat-card theme. Single self-contained file.
import React, {
  useState,
  useMemo,
  useCallback,
...
// ==========================================================
// RECENT LOSS EVENTS MODAL — full-page, navy header, matches Mould
// Change's RecentMouldChangesModal pattern but restyled + flat/sharp.
// ==========================================================
function LossRecentModal({ open, onClose, date, events, loading }) {
  if (!open) return null;
  const rows = toArray(events);

  const durationStyle = (mins) => {
    if (mins >= 45) return "bg-red-50 border-red-200 text-red-600";
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-white">
      <div className="flex flex-shrink-0 items-center justify-between bg-[#0F1D24] px-5 py-3">
        <div className="flex items-center gap-2.5">
          <div className="flex h-8 w-8 items-center justify-center bg-[#FDC94D]">
            <HiOutlineClock className="h-4 w-4 text-[#0F1D24]" />
          </div>
          <div>
            <h2 className="text-[14px] font-extrabold uppercase tracking-wide text-white">
              Recent Loss Events
            </h2>
            <p className="text-[10px] font-medium text-white/50">{date ? `Date: ${formatDisplayDate(date)}` : "All dates"}</p>
          </div>
        </div>

        <div className="flex items-center gap-3">
          <div className="border border-white/15 bg-white/5 px-2.5 py-1.5">
            <p className="text-[8px] font-bold uppercase tracking-wide text-white/50">Events</p>
            <h3 className="font-mono text-[13px] font-extrabold text-[#FDC94D]">{rows.length}</h3>
          </div>
          <button
            onClick={onClose}
            className="flex h-8 w-8 items-center justify-center border border-white/15 text-white/70 transition-colors duration-100 hover:border-white/30 hover:text-white"
          >
      ...
      {loading && (
        <div className="flex-shrink-0 border-b border-[#C6C6C6] px-5 py-1.5 text-[11px] text-[#9B9B9B]">Loading…</div>
      )}
      {!loading && rows.length === 0 && (
        <div className="flex-shrink-0 border-b border-amber-200 bg-amber-50 px-5 py-1.5 text-[11px] font-semibold text-amber-700">
          No loss events recorded for the selected filters.
        </div>
      )}
      ...
      <div className="min-h-0 flex-1 overflow-auto px-5 py-4">
        <div className={`overflow-hidden ${SURFACE}`}>
          <table className="w-full min-w-[860px] border-collapse">
            <thead>
              <tr className="border-b border-[#C6C6C6] bg-[#FAFAFB]">
                <th className="px-3 py-2 text-left text-[9px] font-bold uppercase tracking-wide text-[#9B9B9B]">#</th>
                <th className="px-3 py-2 text-left text-[9px] font-bold uppercase tracking-wide text-[#9B9B9B]">Reason</th>
                <th className="px-3 py-2 text-left text-[9px] font-bold uppercase tracking-wide text-[#9B9B9B]">Hall / Machine</th>
                <th className="px-3 py-2 text-left text-[9px] font-bold uppercase tracking-wide text-[#9B9B9B]">Operator</th>
                <th className="px-3 py-2 text-center text-[9px] font-bold uppercase tracking-wide text-[#9B9B9B]">Time</th>
                <th className="px-3 py-2 text-center text-[9px] font-bold uppercase tracking-wide text-[#9B9B9B]">Shift</th>
                <th className="px-3 py-2 text-center text-[9px] font-bold uppercase tracking-wide text-[#9B9B9B]">Duration</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((item, index) => (
                <tr key={item.id || index} className="border-b border-[#F0F0F0] last:border-b-0 hover:bg-[#FAFAFB]">
                  <td className="px-3 py-2.5 font-mono text-[10px] text-[#9B9B9B]">{index + 1}</td>
                  <td className="px-3 py-2.5">
                    <div className="text-[11px] font-bold text-[#0F1D24]">{item.reason}</div>
                    {item.remarks && <div className="mt-0.5 truncate text-[9.5px] text-[#9B9B9B]">{item.remarks}</div>}
                  </td>
                  <td className="px-3 py-2.5">
                    <div className="flex items-center gap-1 text-[10.5px] font-semibold text-[#0F1D24]">
                      <HiOutlineBuildingOffice2 className="h-3 w-3 flex-shrink-0 text-[#9B9B9B]" />
                      {item.hall || "—"}
                    </div>
                    <div className="mt-0.5 flex items-center gap-1 text-[9.5px] text-[#9B9B9B]">
                      <HiOutlineCog6Tooth className="h-3 w-3 flex-shrink-0" />
                      {item.machine || "—"}
                    </div>
                  </td>
                  <td className="px-3 py-2.5">
                    <div className="flex items-center gap-1 text-[10.5px] font-semibold text-[#0F1D24]">
                      <HiOutlineUser className="h-3 w-3 flex-shrink-0 text-[#9B9B9B]" />
                      {item.operator || "—"}
                    </div>
                  </td>
                  <td className="px-3 py-2.5 text-center font-mono text-[10.5px] text-[#0F1D24]">
                    {item.startTime || "—"}{item.endTime ? ` – ${item.endTime}` : ""}
                  </td>
                  <td className="px-3 py-2.5 text-center text-[10.5px] font-semibold text-[#0F1D24]">{item.shift || "—"}</td>
                  <td className="px-3 py-2.5 text-center">
                    <span className={`inline-block border px-2 py-0.5 font-mono text-[9.5px] font-bold ${durationStyle(item.lossMinutes || 0)}`}>
                      {fmt(item.lossMinutes)} min
                    </span>
                  </td>
                </tr>
              ))}
...
// ==========================================================
// MACHINE HEATMAP MODAL — full-page, machine × hour grid.
// ==========================================================
function LossHeatmapModal({ open, onClose, date, heatMapData, machines }) {
  if (!open) return null;

  const data = toArray(heatMapData);
  const machineList = toArray(machines).map((m) =>
    typeof m === "string" ? { name: m, hall: "Other" } : { name: m.name || m.machine_name || m.machine, hall: m.hall || "Other" },
  );

  const getLoss = (machine, hour) => {
    const rec = data.find((d) => d.machine === machine && d.hour === hour);
    return rec ? rec.lossMinutes : 0;
  };

  const hasData = data.some((d) => d.lossMinutes > 0);

  const getColor = (v) => {
    if (!v) return "#F5F5F5";
    if (v <= 10) return "#D1FAE5";
    if (v <= 20) return "#FDE9B8";
    if (v <= 40) return "#FB923C";
    return "#DC2626";
  };
  const getTextColor = (v) => (v > 20 ? "#FFFFFF" : "#0F1D24");
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-white">
      <div className="flex flex-shrink-0 items-center justify-between bg-[#0F1D24] px-5 py-3">
        <div className="flex items-center gap-2.5">
          <div className="flex h-8 w-8 items-center justify-center bg-[#FDC94D]">
            <HiOutlineSquares2X2 className="h-4 w-4 text-[#0F1D24]" />
          </div>
          <div>
            <h2 className="text-[14px] font-extrabold uppercase tracking-wide text-white">Machine Loss Heatmap</h2>
            <p className="text-[10px] font-medium text-white/50">
              {machineList.length} machines · {date ? formatDisplayDate(date) : "All dates"}
            </p>
          </div>
        </div>
        <button
      ...
      {!hasData && (
        <div className="flex-shrink-0 border-b border-amber-200 bg-amber-50 px-5 py-1.5 text-[11px] font-semibold text-amber-700">
          No downtime recorded for this date — showing all {machineList.length} machines with 0.
        </div>
      )}
      ...
      <div className="min-h-0 flex-1 overflow-auto px-5 py-4">
        <table className="w-full min-w-[900px] table-fixed border-collapse">
          <thead>
            <tr>
              <th className="sticky left-0 top-0 z-30 w-32 border border-[#C6C6C6] bg-[#FAFAFB] px-2 py-1 text-left text-[9px] font-bold uppercase text-[#9B9B9B]">
                Machine
              </th>
              {HOURS_24.map((hour) => (
                <th key={hour} className="sticky top-0 z-20 h-6 w-8 border border-[#C6C6C6] bg-[#FAFAFB] text-[9px] font-bold text-[#9B9B9B]">
                  {hour}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {groups.map(([hall, hallMachines]) => (
              <React.Fragment key={hall}>
                <tr>
                  <td colSpan={25} className="sticky left-0 z-20 border border-[#C6C6C6] bg-[#0F1D24] px-2 py-1 text-[10px] font-bold uppercase tracking-wide text-[#FDC94D]">
                    {hall} · {hallMachines.length} machines
                  </td>
                </tr>
                {hallMachines.map((m) => (
                  <tr key={m.name} className="hover:bg-[#FAFAFB]">
                    <td className="sticky left-0 z-10 border border-[#C6C6C6] bg-white px-2 py-1 text-[9.5px] font-semibold text-[#0F1D24]">
                      {m.name}
                    </td>
                    {HOURS_24.map((hour) => {
                      const value = getLoss(m.name, hour);
                      return (
                        <td
                          key={hour}
                          title={`${m.name} · ${hour}:00 · ${value} min`}
                          className="h-6 w-8 border border-[#C6C6C6] text-center text-[9px] font-bold"
                          style={{ background: getColor(value), color: getTextColor(value) }}
                        >
                          {value || "-"}
                        </td>
                      );
            ...
            {machineList.length === 0 && (
              <tr>
                <td colSpan={25} className="py-6 text-center text-[10px] text-[#9B9B9B]">
                  No machines found.
                </td>
              </tr>
            )}
      ...
      <div className="flex flex-shrink-0 flex-wrap items-center gap-3 border-t border-[#C6C6C6] bg-[#FAFAFB] px-5 py-2 text-[9.5px] text-[#9B9B9B]">
        <span className="flex items-center gap-1"><span className="h-2.5 w-2.5 border border-[#C6C6C6]" style={{ background: "#F5F5F5" }} /> No Loss</span>
        <span className="flex items-center gap-1"><span className="h-2.5 w-2.5" style={{ background: "#D1FAE5" }} /> Low</span>
        <span className="flex items-center gap-1"><span className="h-2.5 w-2.5" style={{ background: "#FDE9B8" }} /> Medium</span>
        <span className="flex items-center gap-1"><span className="h-2.5 w-2.5" style={{ background: "#FB923C" }} /> High</span>
        <span className="flex items-center gap-1"><span className="h-2.5 w-2.5" style={{ background: "#DC2626" }} /> Critical</span>
      </div>
    </div>
  );
}
...
const LossTimeDashboard = () => {
  ...
  return (
    <div className="flex h-screen max-h-screen overflow-hidden bg-[#EFEFEF]">
      ...
      {/* Recent Loss Events — same modal pattern as MouldChangeDashboard's
          RecentMouldChangesModal, restyled to the navy/gold flat theme. */}
      <LossRecentModal
        open={isRecentOpen}
      ...
      {/* Machine Heatmap — same modal pattern as MouldChangeDashboard's
          MouldChangeHeatmap, restyled to the navy/gold flat theme. */}
      <LossHeatmapModal
        open={isHeatmapOpen}
