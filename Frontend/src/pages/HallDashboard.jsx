...
import {
  Calendar, RefreshCw, RotateCcw, Download, ChevronLeft, ChevronRight,
  ChevronDown, Check, Filter, Expand, X, ArrowLeft, LayoutGrid, AlertTriangle,
} from "lucide-react";

import {
  getHallStats, getHallMachineWise, getHallHourlyTrend,
  getHallShiftSummary, getHallTopRejects, getHallMachines,
...
const parseDateKey = (key) => { if (!key) return new Date(); const [y, m, d] = key.split("-").map(Number); return new Date(y, (m || 1) - 1, d || 1); };
const formatDisplay = (key) => !key ? "Select date" : parseDateKey(key).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" });
...
const defaultFilters = () => ({ date: getBusinessDateDefault(), machine: "All", shift: "All" });
const easeOutCubic = (t) => 1 - Math.pow(1 - t, 3);

// ==========================================================
// Inline icons (KPI cards)
// ==========================================================
const IconTarget = (p) => (<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" {...p}><circle cx="12" cy="12" r="8" /><circle cx="12" cy="12" r="4" /><circle cx="12" cy="12" r="0.8" fill="currentColor" /></svg>);
const IconTrendUp = (p) => (<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...p}><path d="M3 17l6-6 4 4 8-8" /><path d="M15 7h6v6" /></svg>);
const IconAlert = (p) => (<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...p}><path d="M12 9v4M12 17h.01" /><circle cx="12" cy="12" r="9" /></svg>);
const IconGauge = (p) => (<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...p}><path d="M12 14l4-4M4 15a8 8 0 1 1 16 0" /></svg>);
const IconAward = (p) => (<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...p}><circle cx="12" cy="8" r="5" /><path d="M8.5 12.5L7 21l5-2.5L17 21l-1.5-8.5" /></svg>);
const IconBarChart = (p) => (<svg viewBox="0 0 24 24" fill="none" {...p}><rect x="3" y="12" width="4" height="9" rx="1" fill="currentColor" /><rect x="10" y="7" width="4" height="14" rx="1" fill="currentColor" /><rect x="17" y="3" width="4" height="18" rx="1" fill="currentColor" /></svg>);
...
const CustomDatePicker = ({ value, onChange }) => {
  ...
  return (
    <div ref={wrapperRef} className="relative flex-shrink-0">
      ...
      <div className={`absolute left-0 top-9 z-50 w-60 origin-top overflow-hidden rounded-sm border border-[#C6C6C6]/60 bg-white shadow-xl transition-all duration-150 ease-out ${open ? "pointer-events-auto scale-100 opacity-100" : "pointer-events-none scale-95 opacity-0"}`}>
        <div className="flex items-center justify-between bg-[#0F1D24] px-2 py-1.5">
          <button type="button" onClick={() => changeMonth(-1)} className="flex h-5 w-5 items-center justify-center rounded-sm text-[#FDC94D] transition hover:bg-white/10"><ChevronLeft size={12} /></button>
          <span className="text-[11px] font-bold text-white">{MONTH_NAMES[viewDate.getMonth()]} {viewDate.getFullYear()}</span>
          <button type="button" onClick={() => changeMonth(1)} className="flex h-5 w-5 items-center justify-center rounded-sm text-[#FDC94D] transition hover:bg-white/10"><ChevronRight size={12} /></button>
        </div>

        <div className="grid grid-cols-7 gap-0.5 px-1.5 pt-1.5">
          {WEEKDAYS.map((w, i) => <div key={`${w}-${i}`} className="flex h-5 items-center justify-center text-[9px] font-semibold text-[#9B9B9B]">{w}</div>)}
        </div>
        ...
        <div className="grid grid-cols-7 gap-0.5 px-1.5 pb-1.5">
          {calendarDays.map((date, i) => {
            ...
            return (
              <button type="button" key={key} onClick={() => handleSelect(date)}
        ...
        <div className="flex items-center justify-between border-t border-[#C6C6C6]/40 bg-[#F5F5F5]/70 px-2 py-1">
          <button type="button" onClick={() => handleSelect(new Date())} className="text-[10px] font-semibold text-[#0F1D24] hover:text-[#FDC94D]">Today</button>
          <button type="button" onClick={() => setOpen(false)} className="text-[10px] font-medium text-[#9B9B9B] hover:text-[#0F1D24]">Close</button>
        </div>
      </div>
    </div>
  );
...
// ==========================================================
// Custom themed select — now works on {value, label} options
// so the machine dropdown can show machine_name while still
// filtering by machine_code.
// ==========================================================
const CustomSelect = ({ value, onChange, options = [], icon: Icon, maxWidth = 190 }) => {
  ...
  return (
    <div ref={wrapperRef} className="relative flex-shrink-0">
      <button type="button" onClick={() => setOpen((o) => !o)} style={{ maxWidth }}
        ...
        <ChevronDown size={10} className={`shrink-0 text-[#9B9B9B] transition-transform ${open ? "rotate-180" : ""}`} />
      </button>

      <div className={`absolute left-0 top-9 z-50 max-h-56 w-64 origin-top overflow-y-auto rounded-sm border border-[#C6C6C6]/60 bg-white py-1 shadow-xl transition-all duration-150 ease-out ${open ? "pointer-events-auto scale-100 opacity-100" : "pointer-events-none scale-95 opacity-0"}`}>
        {options.map((opt) => (
          <button type="button" key={opt.value} onClick={() => handleSelect(opt)}
        ...
        {options.length === 0 && <p className="px-2.5 py-1.5 text-[10px] text-[#9B9B9B]">No options available</p>}
      </div>
    </div>
  );
};
...
// ==========================================================
const FilterBar = ({ draft, setDraft, onApply, onRefresh, onReset, onExport, onHeatmap, onBack, loading, machineList }) => {
  const machineOptions = useMemo(
    () => [
      { value: "All", label: "All Machines" },
      ...machineList.map((m) => ({ value: m.machine_code, label: m.machine_name || m.machine_code })),
    ],
    [machineList]
  );
  const shiftOptions = [
  ...
  return (
    <div className="flex min-h-10 flex-shrink-0 flex-wrap items-center gap-2 rounded-sm border border-[#C6C6C6]/50 bg-white px-2 py-1.5 shadow-sm">
      ...
      <CustomSelect value={draft.machine} onChange={(machine) => setDraft((p) => ({ ...p, machine }))} options={machineOptions} maxWidth={210} />
      <CustomSelect value={draft.shift} onChange={(shift) => setDraft((p) => ({ ...p, shift }))} options={shiftOptions} />

      <div className="flex flex-shrink-0 items-center gap-1.5">
        <button onClick={onApply} title="Apply selected filters"
          ...
          <Filter size={13} /><span className="hidden md:inline">Apply</span>
        </button>
        <button onClick={onRefresh} disabled={loading} title="Refresh data"
        ...
        <button onClick={onReset} title="Reset filters"
          ...
          <RotateCcw size={13} /><span className="hidden md:inline">Reset</span>
        </button>
      </div>
...
const buildHourlyChartData = (trend) => {
  ...
  return ORDERED_HOURS.map((hour) => {
    ...
    return { hour, label: `${String(hour).padStart(2, "0")}:00`, target: d.target || 0, actual: d.actual || 0, shift: SHIFT_A_HOURS.includes(hour) ? "A" : "B" };
  });
};
...
const Chart = ({ chartData }) => {
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver((entries) => {
      for (const entry of entries) setSize({ width: entry.contentRect.width, height: Math.max(entry.contentRect.height, MIN_CHART_HEIGHT) });
    });
    observer.observe(el);
  ...
  return (
    <div ref={containerRef} className="relative h-full min-h-0 w-full">
      <svg viewBox={`0 0 ${width} ${height}`} width="100%" height="100%" className="block" preserveAspectRatio="none">
        ...
        {shiftSegments.map((seg, i) => {
          ...
          return (
            <g key={`seg-label-${i}`}>
              ...
              <text x={cx} y={PADDING.top - (compact ? 30 : 44) + pillH / 2 + 3} textAnchor="middle" fontSize={compact ? "8.5" : "10"} fontWeight="700" fill={seg.shift === "A" ? "#0F1D24" : "#FDC94D"}>Shift {seg.shift}</text>
            </g>
          );
        })}
        {yTicks.map((tick, i) => {
        ...
        {groups.map((g, i) => (
          <g key={i}>
            <rect x={g.barX1} y={g.y1} width={g.barW} height={g.h1} rx={2} fill={BAR_COLORS.target}
            ...
            <rect x={g.barX2} y={g.y2} width={g.barW} height={g.h2} rx={2} fill={BAR_COLORS.actual}
              opacity={hoverIdx === null || hoverIdx === i ? 1 : 0.55}
        ...
        {!compact && (
          <text x={12} y={PADDING.top + chartH / 2} textAnchor="middle" fontSize="10" fontWeight="700" fill="#0F1D24" transform={`rotate(-90, 12, ${PADDING.top + chartH / 2})`}>Qty</text>
        )}
        {groups.map((g, i) => (
      ...
      {hovered && (
        <div className="pointer-events-none absolute z-10 rounded-sm border border-[#C6C6C6]/60 bg-white px-2 py-1.5 text-[10px] shadow-md"
          ...
          <div className="mb-1 flex items-center justify-between gap-3">
            ...
            <span className="rounded-full px-1.5 py-0.5 text-[9px] font-semibold" style={{ background: SHIFT_COLORS[hovered.shift].swatch, color: hovered.shift === "A" ? "#0F1D24" : "#FDC94D" }}>Shift {hovered.shift}</span>
          </div>
          <div className="flex items-center justify-between gap-3">
            <span className="flex items-center gap-1 text-[#9B9B9B]"><span className="h-1.5 w-1.5 rounded-sm" style={{ background: BAR_COLORS.target }} /> Target</span>
            <span className="font-mono font-semibold text-[#0F1D24]">{hovered.target}</span>
          </div>
          <div className="flex items-center justify-between gap-3">
            <span className="flex items-center gap-1 text-[#9B9B9B]"><span className="h-1.5 w-1.5 rounded-sm" style={{ background: BAR_COLORS.actual }} /> Actual</span>
...
const HourlyChartCard = ({ chartData, loading }) => {
  ...
  const totalTarget = useMemo(() => chartData.reduce((s, d) => s + d.target, 0), [chartData]);
  const totalActual = useMemo(() => chartData.reduce((s, d) => s + d.actual, 0), [chartData]);
  const hasData = totalTarget + totalActual > 0;

  const peak = useMemo(() => {
    let best = { hour: "-", value: 0 };
    chartData.forEach((d) => { if (d.actual > best.value) best = { hour: d.hour, value: d.actual }; });
    return best;
  }, [chartData]);
      ...
      <div className="flex min-h-0 flex-1 flex-col rounded-sm border border-[#C6C6C6]/60 bg-white p-2.5 shadow-[0_1px_2px_rgba(15,29,36,0.04)]">
        <div className="mb-1 flex flex-shrink-0 flex-wrap items-center justify-between gap-2 pr-1">
          <div className="flex items-center gap-2">
            <div className="flex h-7 w-7 items-center justify-center rounded-sm bg-[#0F1D24]"><IconBarChart className="h-3 w-3 text-[#FDC94D]" /></div>
            <div>
          ...
          <div className="flex items-center gap-3">
            <div className="flex flex-wrap items-center gap-2">
              <div className="flex items-center gap-1.5 rounded-full border border-[#0F1D24]/20 bg-[#0F1D24]/5 px-2 py-0.5">
                <span className="h-2 w-2 rounded-sm" style={{ background: BAR_COLORS.target }} /><span className="text-[10px] font-semibold text-[#0F1D24]">Target</span>
              </div>
              <div className="flex items-center gap-1.5 rounded-full border border-[#FDC94D]/50 bg-[#FDC94D]/10 px-2 py-0.5">
                <span className="h-2 w-2 rounded-sm" style={{ background: BAR_COLORS.actual }} /><span className="text-[10px] font-semibold text-[#0F1D24]">Actual</span>
              </div>
            </div>
            ...
            <button onClick={() => setIsZoomed(true)} className="flex h-6 items-center gap-1 rounded-sm bg-[#0F1D24] px-2 text-[9px] font-semibold text-[#FDC94D] transition hover:bg-[#1a2e38]">
              <Expand size={11} /> Zoom
            </button>
          </div>
        </div>
        ...
        {loading ? (
          <div className="flex h-full min-h-[160px] items-center justify-center text-[11px] text-[#9B9B9B]">Loading hourly data...</div>
        ) : (
          <div className="flex h-full min-h-0 flex-col">
            ...
            <div className="min-h-0 flex-1"><Chart chartData={chartData} /></div>
      ...
      {isZoomed && (
        <div className="fixed inset-0 z-50 flex flex-col bg-white">
          <div className="flex flex-shrink-0 items-center justify-between border-b border-[#C6C6C6]/50 px-5 py-3">
            <div className="flex items-center gap-2">
              <div className="flex h-8 w-8 items-center justify-center rounded-sm bg-[#0F1D24]"><IconBarChart className="h-3.5 w-3.5 text-[#FDC94D]" /></div>
              <div>
            ...
            <div className="flex items-center gap-4">
              ...
              <button onClick={() => setIsZoomed(false)} className="flex h-8 w-8 items-center justify-center rounded-sm text-[#9B9B9B] transition hover:bg-[#0F1D24]/5">
                <X size={16} />
              </button>
            </div>
          </div>
          ...
          <div className="min-h-0 flex-1 px-6 py-4"><Chart chartData={chartData} /></div>
          ...
          <div className="flex flex-shrink-0 items-center justify-between border-t border-[#C6C6C6]/40 bg-[#0F1D24]/[0.02] px-5 py-2">
            <div className="flex items-center gap-3 text-[10px]">
              <span className="flex items-center gap-1 text-[#9B9B9B]"><span className="h-2 w-2 rounded-sm" style={{ background: BAR_COLORS.target }} /> Target</span>
              <span className="flex items-center gap-1 text-[#9B9B9B]"><span className="h-2 w-2 rounded-sm" style={{ background: BAR_COLORS.actual }} /> Actual</span>
            </div>
...
const HallDashboard = () => {
  ...
  const fetchAll = useCallback(async () => {
    ...
    if (statsRes.status === "fulfilled" && statsRes.value.success) setStats(statsRes.value.data); else failures.push("stats");
    if (machineWiseRes.status === "fulfilled" && machineWiseRes.value.success) setMachineWise(machineWiseRes.value.data); else failures.push("machine-wise");
    if (hourlyRes.status === "fulfilled" && hourlyRes.value.success) setHourlyTrend(hourlyRes.value.data.trend); else failures.push("hourly-trend");
    if (machinesRes.status === "fulfilled" && machinesRes.value.success) setMachines(machinesRes.value.data); else failures.push("machines");
  ...
  return (
    <div className="relative h-screen overflow-hidden bg-[#F5F5F5]">
      ...
      <div className="relative z-10 mx-auto flex h-full max-w-[1800px] flex-col gap-2 px-1 py-2 sm:px-2 lg:px-2">
        {error && <div className="flex-shrink-0 rounded-sm border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-700">{error}</div>}
