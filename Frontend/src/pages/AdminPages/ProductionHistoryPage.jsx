  ...
  return (
    <div ref={ref} className={`relative ${className}`}>
      ...
      >
        <span
          className={
            selected ? "truncate text-[#0F1D24]" : "truncate text-[#9B9B9B]"
          }
        >
          {selected ? selected.label : placeholder}
        </span>
        <HiOutlineChevronDown
          className={`h-3.5 w-3.5 shrink-0 text-[#9B9B9B] transition-transform duration-100 ${open ? "rotate-180" : ""}`}
        />
      </button>
      {open && (
...
function ThemedDatePicker({ value, onChange, className = "" }) {
  ...
  for (let i = startOffset - 1; i >= 0; i--) {
    cells.push({
      ...
      key: toDateKey(new Date(year, month - 1, daysInPrevMonth - i)),
      inMonth: false,
    });
  }
  for (let d = 1; d <= daysInMonth; d++)
    cells.push({
      day: d,
      key: toDateKey(new Date(year, month, d)),
      inMonth: true,
    });
  let extra = 1;
  while (cells.length < 42) {
    cells.push({
      day: extra,
      key: toDateKey(new Date(year, month + 1, extra)),
      inMonth: false,
    });
    extra++;
  }
  ...
  return (
    <div ref={ref} className={`relative ${className}`}>
      ...
      >
        <span
          className={
            value ? "truncate text-[#0F1D24]" : "truncate text-[#9B9B9B]"
          }
        >
      ...
      {open && (
        <div className="absolute z-30 mt-1 w-64 border border-[#C6C6C6] bg-white shadow-[0_4px_10px_rgba(15,29,36,0.12)]">
          <div className="flex items-center justify-between border-b border-[#C6C6C6] bg-[#FAFAFA] px-2 py-1.5">
            ...
            <span className="text-[11.5px] font-bold text-[#0F1D24]">
              {monthLabel}
            </span>
            <button
              type="button"
          ...
          <div className="grid grid-cols-7 border-b border-[#C6C6C6] bg-white">
            {WEEKDAY_LABELS.map((w) => (
              <div
                key={w}
                className="py-1 text-center text-[9.5px] font-bold uppercase text-[#9B9B9B]"
              >
                {w}
              </div>
            ))}
    ...
    <div className="flex border border-[#C6C6C6] bg-white">
      <div className="flex flex-1 items-center justify-between px-2.5 py-2">
        <div className="min-w-0">
          ...
          <p
            className={`mt-0.5 font-mono text-base font-extrabold leading-none ${t.text}`}
          >
            {value}
          </p>
        </div>
        <div
          className={`flex h-7 w-7 shrink-0 items-center justify-center border border-[#C6C6C6] ${t.text}`}
        >
          {icon}
        </div>
      </div>
...
const BreakdownPanel = ({
  ...
  return (
    <div className="border border-[#C6C6C6] bg-white">
      ...
      <div className="p-3">
        {rows.length === 0 ? (
          <p className="py-4 text-center text-[11px] text-[#9B9B9B]">
            No data available.
          </p>
        ) : (
...
const AdminProductionHistoryPage = () => {
  ...
  const exportToExcel = () => {
    ...
    const wsEntries = XLSX.utils.json_to_sheet(
      entries.map((e) => ({
        ...
        Part: e.part_name,
        "Standard CT": e.standard_cycle_time,
        "Actual CT": e.actual_cycle_time,
        Target: e.target_qty,
        Actual: e.actual_qty,
        Good: e.good_qty,
        Reject: e.reject_qty,
    ...
    XLSX.writeFile(
      wb,
      `Production_Report_${appliedFilters.fromDate}_to_${appliedFilters.toDate}.xlsx`,
    );
  };
            ...
            <div className="grid grid-cols-2 gap-2 p-3 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-7">
              <div className="relative z-20">
                ...
                <ThemedDatePicker
                  ...
                  onChange={(v) => updateDraft("fromDate", v)}
                />
              </div>
              <div className="relative z-20">
                <label className="mb-1 block text-[9.5px] font-mono uppercase tracking-wide text-[#9B9B9B]">
                ...
                <ThemedDatePicker
                  ...
                  onChange={(v) => updateDraft("toDate", v)}
                />
              </div>
              <div>
                <label className="mb-1 block text-[9.5px] font-mono uppercase tracking-wide text-[#9B9B9B]">
              ...
              <div>
                ...
                <ThemedSelect
                  ...
                  onChange={(v) => updateDraft("shift", v)}
                  options={[
                    { value: "All", label: "All Shifts" },
                    ...SHIFTS.map((s) => ({ value: s, label: `Shift ${s}` })),
                  ]}
                />
              </div>
              <div>
                ...
                <ThemedSelect
                  ...
                  onChange={(v) => updateDraft("rejectReasonId", v)}
                  options={[
                    { value: "", label: "All Reject Reasons" },
              ...
              <div>
                ...
                <ThemedSelect
                  ...
                  onChange={(v) => updateDraft("lossReasonId", v)}
                  options={[
                    { value: "", label: "All Loss Reasons" },
                    ...lossReasons.map((r) => ({
                      value: r.id,
                      label: r.reason_name,
                    })),
              ...
              <div>
                ...
                <ThemedSelect
                  ...
                  onChange={(v) => updateDraft("mouldChange", v)}
                  options={MOULD_CHANGE_OPTIONS}
                />
              </div>
            </div>
          </div>
            ...
            <div id="report-print-area" className="mx-3">
              ...
              <div className="hidden print:block">
                <h2 className="text-lg font-bold text-[#0F1D24]">
                  Dixon MPM — Production Report
                </h2>
                <p className="text-xs text-[#9B9B9B]">
                  {formatDateLabel(appliedFilters.fromDate)} to{" "}
                  {formatDateLabel(appliedFilters.toDate)} ·{" "}
                  {appliedFilters.hall} · Shift {appliedFilters.shift}
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-7">
                <SummaryTile
                  ...
                  icon={<FaIndustry className="text-[13px]" />}
                  tone="slate"
                />
                <SummaryTile
                  label="Target"
                  value={summary?.totalTarget ?? 0}
                  icon={<FaBoxes className="text-[13px]" />}
                  tone="blue"
                />
                <SummaryTile
                  label="Actual"
                ...
                <SummaryTile
                  ...
                  icon={<FaExclamationTriangle className="text-[13px]" />}
                  tone="red"
                />
                <SummaryTile
                  label="Loss (min)"
                  value={summary?.totalLossMinutes ?? 0}
                  icon={<FaClock className="text-[13px]" />}
                  tone="orange"
                />
                <SummaryTile
                  label="Avg Efficiency"
                  value={`${summary?.avgEfficiency ?? 0}%`}
                  icon={<FaChartLine className="text-[13px]" />}
                  tone="gold"
                />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3">
                <SummaryTile
                  label="Mould Changes"
                  value={mouldChangeStats?.totalChanges ?? 0}
                  icon={<FaTools className="text-[13px]" />}
                  tone="purple"
                />
                <SummaryTile
                    ...
                    <tbody>
                      {entries.length === 0 ? (
                        ...
                        entries.map((entry) => (
                          <tr
                            key={entry.id}
                            className="border-t border-[#C6C6C6] hover:bg-[#FAFAFA]"
                          >
                            <td className="px-2.5 py-1.5 font-mono text-[#0F1D24]">
                            ...
                            <td className="px-2.5 py-1.5 text-[#9B9B9B]">
                              {entry.shift} · {entry.time_slot}
                            </td>
                            <td className="px-2.5 py-1.5 text-right font-mono text-blue-700">
                              {entry.target_qty}
                            </td>
                            <td className="px-2.5 py-1.5 text-right font-mono text-emerald-700">
                              {entry.actual_qty}
                            </td>
                            <td className="px-2.5 py-1.5 text-right font-mono text-teal-700">
                              {entry.good_qty}
                            </td>
                            <td className="px-2.5 py-1.5 text-right font-mono text-red-700">
                              {entry.reject_qty}
                            </td>
                            <td className="px-2.5 py-1.5 text-right font-mono text-orange-700">
                              {entry.loss_minutes}
                            </td>
                            <td className="px-2.5 py-1.5 text-right font-mono font-bold text-[#0F1D24]">
                              {entry.efficiency}%
                            </td>
                          </tr>
                        ))
...
export default AdminProductionHistoryPage;