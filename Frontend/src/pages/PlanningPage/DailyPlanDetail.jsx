...
// ============================================================
// Stat card — small metric tile used in the detail header
// ============================================================
function StatCard({ icon: Icon, label, value, index }) {
  return (
    <motion.div
      ...
      className="flex flex-1 items-center gap-3 rounded-sm border border-[#C6C6C6] bg-white px-3 py-2.5"
    >
      <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-sm border border-[#C6C6C6] bg-[#F5F5F5] text-[#0F1D24]">
        <Icon className="h-4.5 w-4.5" />
      </div>
      <div className="min-w-0">
        <p className="font-mono text-[10px] uppercase tracking-wide text-[#9B9B9B]">{label}</p>
        <p className="text-base font-bold leading-tight text-[#0F1D24]">{value}</p>
      </div>
    </motion.div>
  );
}
...
// Main component
// ============================================================
export default function DailyProductionPlanDetail() {
  const {
  ...
  // ---- Inline row editing state (existing plan, Draft status only) ----
  const [editingId, setEditingId] = useState(null);
  const [editDraft, setEditDraft] = useState({ monthlyDetailId: "", targetQty: "", plannedCycleTime: "" });
  ...
  const startEditRow = (row) => {
    setRowError("");
    setEditingId(row.daily_detail_id);
    setEditDraft({
      monthlyDetailId: row.monthly_detail_id || "",
      targetQty: row.target_qty ?? "",
      plannedCycleTime: row.planned_cycle_time ?? "",
    });
  };
  ...
  const saveEditRow = async (row) => {
    setRowError("");
    if (!editDraft.monthlyDetailId) return setRowError("Select a part");
    if (!editDraft.targetQty) return setRowError("Target qty is required");

    setSavingRowId(row.daily_detail_id);
    try {
      const md = monthlyParts.find((p) => p.monthly_detail_id === Number(editDraft.monthlyDetailId));
      await updateRow(row.daily_detail_id, {
        monthlyDetailId: editDraft.monthlyDetailId,
        partId: md?.part_id || row.part_id || null,
        targetQty: Number(editDraft.targetQty),
        plannedCycleTime: editDraft.plannedCycleTime ? Number(editDraft.plannedCycleTime) : null,
      });
      setEditingId(null);
    } catch (err) {
      setRowError(err.response?.data?.message || "Failed to update row");
    } finally {
      setSavingRowId(null);
    }
  };
  ...
  // ==========================================================
  // View 3 — Existing plan: stat cards + editable machine table + mould changes
  // ==========================================================
  return (
    <div className="min-h-screen space-y-2 bg-[#F5F5F5] p-2">
      <motion.div
      ...
      <div className="flex flex-col gap-2 sm:flex-row">
        <StatCard icon={HiOutlineCube} label="Total Machines" value={header.total_machines} index={0} />
        <StatCard icon={HiOutlineClipboardDocumentCheck} label="Planned Machines" value={header.planned_machines} index={1} />
        <StatCard icon={HiOutlineChartBarSquare} label="Total Target Qty" value={header.total_target_qty} index={2} />
      </div>

      {rowError && <p className="text-[11.5px] font-semibold text-red-600">{rowError}</p>}
        ...
        <table className="w-full min-w-[880px] text-[12px]">
          ...
          <tbody>
            {details.map((row) => {
              ...
              const isEditing = editingId === row.daily_detail_id;
              const isSaving = savingRowId === row.daily_detail_id;

              return (
                <tr key={row.daily_detail_id} className={`border-t border-[#C6C6C6]/60 ${isEditing ? "bg-[#FDC94D]/[0.06]" : ""}`}>
                  ...
                  <td className="px-2.5 py-2">
                    {isEditing ? (
                      <ThemedSelect
                        value={editDraft.monthlyDetailId}
                        onChange={(val) => {
                          const md = monthlyParts.find((p) => p.monthly_detail_id === Number(val));
                          setEditDraft((prev) => ({
                            ...prev,
                            monthlyDetailId: val,
                            plannedCycleTime: md?.planned_cycle_time || md?.actual_cycle_time || prev.plannedCycleTime,
                          }));
                        }}
                        options={monthlyParts.map((p) => ({ value: p.monthly_detail_id, label: `${p.part_number} - ${p.part_name}` }))}
                  ...
                  <td className="px-2.5 py-2 text-right">
                    {isEditing ? (
                      <input
                        type="number" min="1" value={editDraft.targetQty}
                        onChange={(e) => setEditDraft((prev) => ({ ...prev, targetQty: e.target.value }))}
                        className="h-7 w-20 rounded-sm border border-[#C6C6C6] px-1.5 text-right text-[11.5px] font-mono outline-none focus:border-[#0F1D24]"
                      />
                    ) : (
                      <span className="font-mono">{row.target_qty}</span>
                    )}
                  </td>
                  ...
                  <td className="px-2.5 py-2 text-right">
                    {isEditing ? (
                      <input
                        type="number" step="0.01" min="0" value={editDraft.plannedCycleTime}
                        onChange={(e) => setEditDraft((prev) => ({ ...prev, plannedCycleTime: e.target.value }))}
                        className="h-7 w-20 rounded-sm border border-[#C6C6C6] px-1.5 text-right text-[11.5px] font-mono outline-none focus:border-[#0F1D24]"
                      />
                    ) : (
                      <span className="font-mono">{row.planned_cycle_time ?? "-"}</span>
                    )}
                  </td>
                  ...
                  <td className="px-2.5 py-2 text-center">
                    {openChange ? (
                      ...
                      <button
                        ...
                        disabled={isEditing}
                        className="flex h-7 items-center gap-1 rounded-sm border border-[#C6C6C6] px-2 text-[11px] font-semibold text-[#0F1D24] hover:border-[#0F1D24] mx-auto disabled:cursor-not-allowed disabled:opacity-40"
                      >
                        ...
                        {isEditing ? (
                          <>
                            <button
                              ...
                              disabled={isSaving}
                              title="Save"
                              className="flex h-6 w-6 items-center justify-center rounded-sm text-green-600 hover:bg-green-50 disabled:opacity-50"
                            >
                              <HiOutlineCheck className="h-3.5 w-3.5" />
                            </button>
                            <button
                              ...
                              disabled={isSaving}
                              title="Cancel"
                              className="flex h-6 w-6 items-center justify-center rounded-sm text-red-500 hover:bg-red-50 disabled:opacity-50"
                            >
                              <HiOutlineXMark className="h-3.5 w-3.5" />
                            </button>
                          </>
                          ...
                          <>
                            ...
                            <button
                              onClick={() => removeRow(row.daily_detail_id)}
                              title="Delete"
                              className="flex h-6 w-6 items-center justify-center rounded-sm text-red-500 hover:bg-red-50"
                            >
                              <HiOutlineTrash className="h-3.5 w-3.5" />
