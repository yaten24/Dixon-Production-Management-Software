          ...
          >
            <button
              ...
              className={`flex w-full items-center justify-between px-2.5 py-1.5 text-left text-[11px] font-medium transition-colors ${
                value === "" ? "bg-[#FDC94D]/20 text-[#0F1D24]" : "text-[#0F1D24] hover:bg-[#F5F5F5]"
              }`}
            >
              {placeholder}
              {value === "" && <FaCheck className="shrink-0 text-[9px] text-[#0F1D24]" />}
            </button>
...
const LossFilters = ({
  ...
  return (
    <div className="flex min-h-10 flex-shrink-0 flex-wrap items-center gap-2 rounded border border-[#C6C6C6]/50 bg-white px-2 py-1.5 shadow-sm">
      {/* Date + Reason */}
      <CustomDatePicker value={selectedDate} onChange={onDateChange} />
      ...
      <div className="flex flex-shrink-0 items-center gap-1.5 lg:ml-auto">
        <button
          ...
          title="View recent loss time records"
        ...
        <button
          ...
          title="View machine wise loss time heatmap"
