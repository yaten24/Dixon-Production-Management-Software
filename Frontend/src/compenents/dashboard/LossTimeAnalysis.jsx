...
const lossTimeData = [
  { reason: "Machine Breakdown", minutes: 180 },
  { reason: "Mould Change", minutes: 145 },
  { reason: "Material Waiting", minutes: 110 },
  { reason: "Operator Waiting", minutes: 90 },
  { reason: "Quality Check", minutes: 75 },
  { reason: "Power Failure", minutes: 55 },
  { reason: "Machine Setting", minutes: 40 },
  { reason: "Tool Cleaning", minutes: 30 },
];
...
const LossTimeAnalysis = () => {
  ...
  const totalLoss = lossTimeData.reduce((sum, item) => sum + item.minutes, 0);
  const topReason = lossTimeData.reduce((a, b) =>
    a.minutes > b.minutes ? a : b,
  );
  const lossPercentage = ((totalLoss / 1440) * 100).toFixed(1);

  const topFive = [...lossTimeData]
    .sort((a, b) => b.minutes - a.minutes)
    .slice(0, 5);
  const maxMinutes = topFive[0].minutes;
          ...
          <div>
            ...
            <div className="space-y-1.5">
              {topFive.map((item, index) => {
                const pctOfTotal = ((item.minutes / totalLoss) * 100).toFixed(
                  1,
                );
