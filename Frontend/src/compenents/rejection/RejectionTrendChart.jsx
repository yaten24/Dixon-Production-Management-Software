...
const Chart = ({ chartData }) => {
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver((entries) => {
      for (const entry of entries) {
        const w = Math.max(entry.contentRect.width, MIN_WIDTH);
  ...
  const compact = height < 280 || width < 480;
  ...
  // Font sizes scale down one more notch on very narrow containers so
  // labels never visually stretch or overlap. viewBox = exact pixel
  // width/height (no preserveAspectRatio) so nothing ever gets scaled.
  const fs = {
    xLabel: veryCompact ? "6.5" : compact ? "7.5" : "8.5",
    yTick: veryCompact ? "8" : "9",
    barLabel: veryCompact ? "7" : "8",
    axisTitle: veryCompact ? "8" : compact ? "9" : "10",
    shiftPill: veryCompact ? "7.5" : compact ? "8.5" : "10",
  };
    ...
    <div ref={containerRef} className="relative h-full min-h-0 w-full">
      ...
      >
        ...
        {shiftSegments.map((seg, i) => {
          ...
          const pillW = veryCompact ? 42 : compact ? 52 : 66;
        ...
        {bars.map((b, i) => {
          if (veryCompact && i % 2 !== 0) return null;
          return (
            <text
              key={`label-${i}`}
          ...
          <div className="flex h-full min-h-0 flex-col">
            ...
            {!hasData && (
              <div className="mb-1 flex-shrink-0 rounded border border-amber-200 bg-amber-50 px-2 py-1 text-[10px] text-amber-700">
                No rejection entries found for this date — showing 0 for every hour.
              </div>
            )}
            ...
            <div className="mt-1 flex flex-shrink-0 items-center justify-end border-t border-[#C6C6C6]/40 pt-1">
              <span className="text-[9px] font-semibold text-[#9B9B9B]">
                Total Reject Qty: <span className="text-red-600">{totalRejectQty}</span>
              </span>
            </div>
          </div>
        )}
        ...
        <div className="fixed inset-0 z-50 flex flex-col bg-white">
          <div className="flex flex-shrink-0 items-center justify-between border-b border-[#C6C6C6]/50 px-5 py-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h2 className="text-sm font-bold text-[#0F1D24]">Hourly Rejection Trend · Expanded View</h2>
